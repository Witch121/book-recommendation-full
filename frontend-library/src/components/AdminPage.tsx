...
import { collection, getDocs, Timestamp, QueryDocumentSnapshot } from "firebase/firestore";
...
const AdminPage: React.FC = () => {
    ...
    const fetchUsersAndBooks = async () => {
      ...
      try {
        ...
        const usersData = usersSnapshot.docs.map(
          (doc: QueryDocumentSnapshot) => ({ id: doc.id, ...doc.data() })
        ) as User[];
  ...
  const calculateStats = (users: User[], books: Book[]) => {
